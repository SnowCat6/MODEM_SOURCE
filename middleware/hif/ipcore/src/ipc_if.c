 ...
 *----------------------------------------------------------------------------*/
typedef struct _ipc_internal_ior_2_t {
    struct _ipc_internal_ior_2_t    *next_request;
    qbm_gpd*                    first_gpd;
    qbm_gpd*                    last_gpd;
    union {
        kal_uint8                   ebi;
 ...
 * Private variables.
 *----------------------------------------------------------------------------*/
static ipc_internal_ior_2_t     *ipc_send_ul_pkt_ior_head_s = NULL;
static ipc_internal_ior_2_t     *ipc_send_ul_pkt_ior_tail_s = NULL;
...
static void ipc_on_process_send_ul_pkt_queue(ipc_internal_ior_2_t *head_ior)
{
    ipc_internal_ior_2_t       *curr_ior;
    ipc_internal_ior_2_t       *next_ior;

    for (curr_ior = head_ior; curr_ior; curr_ior = next_ior) {
        next_ior = curr_ior->next_request;

        if (curr_ior->ip_type == -1) {
            hif_data_trace(IPC_UL_SEND_PKT, curr_ior->dst_network.ebi, curr_ior->first_gpd, curr_ior->last_gpd);

            /* Sent to UPCM */
            IPC_FORWARD_UL_SDU_BY_EBI(curr_ior->dst_network.ebi, curr_ior->first_gpd, curr_ior->last_gpd);
        } else {
            hif_data_trace(IPC_UL_SEND_PKT_BY_PDN, curr_ior->dst_network.pdn, curr_ior->ip_type, curr_ior->first_gpd, curr_ior->last_gpd);

            /* Sent to UPCM */
            IPC_FORWARD_UL_SDU((kal_uint8)curr_ior->ip_type, curr_ior->dst_network.pdn, curr_ior->first_gpd, curr_ior->last_gpd);
        }
    }
}
...
static void ipc_on_process_ul_queue(void)
{
    ...
    ipc_internal_ior_2_t       *head_ior_2;
    ...
    // For ipc_send_ul_pkt_ior queue
    head_ior_2 = ipc_send_ul_pkt_ior_head_s;
    ipc_send_ul_pkt_ior_head_s = NULL;
    ipc_send_ul_pkt_ior_tail_s = NULL;

    ipc_ul_processing_s = KAL_FALSE;
    RestoreIRQMask(mask);

    ipc_on_process_send_ul_pkt_queue(head_ior_2);
    ...
    kal_uint8   ebi)
{
    ...
    ior = (ipc_internal_ior_2_t *)QBM_DES_GET_SW_CTRL_FIELD(head_gpd);
    ior->next_request = NULL;
    ior->first_gpd = head_gpd;
    ior->last_gpd = tail_gpd;
    ior->dst_network.ebi = ebi;
    ior->ip_type = -1;

    mask = SaveAndSetIRQMask();
    if (ipc_send_ul_pkt_ior_tail_s) {
        ipc_send_ul_pkt_ior_tail_s->next_request = ior;
        ipc_send_ul_pkt_ior_tail_s = ior;
    } else {
        ipc_send_ul_pkt_ior_head_s = ior;
        ipc_send_ul_pkt_ior_tail_s = ior;
    }
